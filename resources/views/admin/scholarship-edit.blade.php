...
@section('title', 'Edit ' . $scholarship->name . ' - SCHOLARIX')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <p class="text-gray-600 mt-2">{{ $scholarship->name }}</p>
    </div>

    <!-- Edit Form -->
    <div class="bg-white rounded-lg shadow p-8">
        <form action="{{ route('admin.scholarship.update', $scholarship) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    >
                    <p class="text-xs text-gray-500 mt-1">Currently filled: {{ $scholarship->slots_filled }}</p>
                    @error('slots')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="category"
                        name="category"
                        value="{{ old('category', $scholarship->category) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    >
                    @error('category')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="4"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                >{{ old('requirements', $scholarship->requirements) }}</textarea>
                @error('requirements')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
